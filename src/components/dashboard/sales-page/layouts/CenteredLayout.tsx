    ...
    <div className="font-sans">
      ...
      <section className="relative py-20 lg:py-28 overflow-hidden">
        <div 
          ...
          style={{ background: `radial-gradient(circle at 50% 0%, ${primaryColor}30, transparent 60%)` }}
        />
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl mx-auto text-center space-y-6">
            ...
            <h1 className={`text-4xl md:text-5xl font-bold leading-tight ${textPrimary}`}>
              {config.hero.title || service.name}
            </h1>

            {(config.hero.subtitle || service.description) && (
              <p className={`text-lg leading-relaxed max-w-2xl mx-auto ${textSecondary}`}>
                {config.hero.subtitle || service.description}
              </p>
            )}
            ...
            <div className="flex flex-wrap justify-center gap-6 py-4">
              {[
                ...
                { icon: Award, value: '✓', label: 'Certificado' },
            ...
            <div className="flex flex-col items-center gap-3 pt-2">
              <Button
                size="lg"
                className="text-base font-semibold px-10 py-6 rounded-xl text-white"
            ...
            {config.hero.showVideo && heroImageUrl && (
              <div className="pt-10">
                <div className={`relative aspect-video rounded-2xl overflow-hidden ${cardBorder} border shadow-xl max-w-2xl mx-auto`}>
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid md:grid-cols-2 gap-4 max-w-2xl mx-auto">
              {config.benefits.items.map((item, i) => (
                <div 
                  key={i} 
                  className={`flex items-center gap-4 p-4 rounded-xl ${cardBg} ${cardBorder} border`}
                >
                  <div 
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
            {modules.map((module, index) => (
              ...
              >
                <div 
                  className="w-10 h-10 rounded-lg flex items-center justify-center mb-3 text-white font-semibold"
                  style={{ backgroundColor: primaryColor }}
                >
                  {index + 1}
                </div>
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          <div className={`max-w-xl mx-auto rounded-2xl p-10 text-center ${cardBg} ${cardBorder} border-2 shadow-lg`}>
            <h2 className={`text-2xl font-bold mb-3 ${textPrimary}`}>{config.cta.mainText}</h2>
            <p className={`mb-6 ${textMuted}`}>{config.cta.subText}</p>
            
            <div className={`text-4xl font-bold mb-6 ${textPrimary}`}>
            ...
            {config.guarantee.enabled && (
              <div className={`mt-6 pt-4 border-t ${cardBorder} flex items-center justify-center gap-2`}>
                <Shield className="w-4 h-4 text-green-500" />
                <span className={`text-sm ${textSecondary}`}>{config.guarantee.title}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <section className="py-16" style={{ backgroundColor: `${primaryColor}05` }}>
          <div className="container mx-auto px-4">
            ...
            <div className="max-w-sm mx-auto text-center">
              <Avatar className="w-24 h-24 mx-auto mb-4 border-4" style={{ borderColor: primaryColor }}>
              ...
              {profile.specialty && <p className={`mt-1 ${textMuted}`}>{profile.specialty}</p>}
